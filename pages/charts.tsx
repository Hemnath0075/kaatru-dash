...
function Charts() {
  ...
  const searchDevices = () =>{
   ...
   const searchMultipleDevices = () =>{
    ...
    const multipleArr:string[] | undefined = [];
    console.log(devicesArr);
    devicesArr?.map((item,index)=>{
      if(item.includes('-')){
        let range=item.split('-');
        if(/\b(SG)\d+\b/.test(range[0])){
          for(let i=parseInt(range[0].slice(2));i<=parseInt(range[1].slice(2));i++){
            // console.log("SG"+i)

            multipleArr.push("SG"+i);
            selectedDeviceData.push({deviceId:"SG"+i,color:colors[index]})
          }
        }
        else if(/\b(MG)\d+\b/.test(range[0])){
          for(let i=parseInt(range[0].slice(2));i<=parseInt(range[1].slice(2));i++){
            multipleArr.push("MG"+i);
            selectedDeviceData.push({deviceId:item,color:colors[index]})
          }
        }
        else if(/\b(LMG)\d+\b/.test(range[0])){
          for(let i=parseInt(range[0].slice(2));i<=parseInt(range[1].slice(2));i++){
            multipleArr.push("LMG"+i);
            selectedDeviceData.push({deviceId:item,color:colors[index]})
          }
        }
      }
  ...
  return (
    <div className='w-[100vh] h-[100vh] flex flex-col'>
      ...
      <ResponsiveContainer width="100%" height="80%">
          ...
          domain={['auto', 'auto']} allowDuplicatedCategory={false} angle={0} tick={({ x, y, payload }) => <CustomXAxisLabel x={x} y={y} payload={payload} />}/>
                      <YAxis
                        domain={
                          chartData?.length > 0 ?
                            type1 ?
                      ...
                      {type1 &&
                        chartData?.map((item:any, index:number) => (
                          ...
                          {/* <Legend verticalAlign="top" height={36} payload={[{value:"custom"},{value:"custom1"}]}/> */}
                          <Legend
  payload={
    chartData?.map(
